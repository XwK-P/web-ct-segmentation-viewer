...
**Core rendering** uses [Niivue](https://github.com/niivue/niivue) (`@niivue/niivue`), a WebGL 2.0 medical image viewer. The entire app lives in a single component at `src/App.tsx` (~1,300 lines) — there are no sub-components, routers, or state management libraries. `src/main.tsx` is just the React root mount.
